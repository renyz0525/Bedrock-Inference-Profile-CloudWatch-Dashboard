...
const ExportUtils = {
  ...
  exportPDF: async function(options) {
    ...
    try {
      // Detect common path from current page's stylesheet links
      var commonPath = '../common/';
      var themeLink = document.querySelector('link[href*="theme.css"]');
      if (themeLink) {
        commonPath = themeLink.getAttribute('href').replace('theme.css', '');
      }

      // Fetch theme CSS and inline it (avoids path/CORS issues)
      this.updateProgress('Fetching theme CSS...', 5);
      var themeCSS = '';
      try {
        var themeResp = await fetch(commonPath + 'theme.css');
        if (themeResp.ok) themeCSS += await themeResp.text();
      } catch(e) {}
      try {
        var overrideResp = await fetch(commonPath + 'theme-override.css');
        if (overrideResp.ok) themeCSS += '\n' + await overrideResp.text();
      } catch(e) {}

      var responses = await Promise.all(
        blocks.map(function(file, i) {
          ExportUtils.updateProgress('Fetching ' + file + '...', 10 + ((i + 1) / blocks.length) * 40);
      ...
      var printHTML = '<!DOCTYPE html>\n<html lang="ko">\n<head>\n' +
        ...
        'html, body { margin: 0; padding: 0; background: #0d1117; overflow: visible !important; display: block !important; height: auto !important; }\n' +
        '.slide-deck { display: block !important; transform: none !important; width: auto !important; height: auto !important; }\n' +
        '.slide, .print-slide {\n' +
        ...
        /* Show all fragments, tabs, compare contents */
        '.fragment { opacity: 1 !important; transform: none !important; visibility: visible !important; }\n' +
        '.tab-content, .compare-content { display: block !important; }\n' +
        '.tab-bar, .compare-toggle, .btn-group { display: none !important; }\n' +
        ...
        '  color: rgba(255,255,255,0.4); font-style: italic; font-size: 1.1rem;\n' +
        '  background: rgba(255,255,255,0.05); border-radius: 0.5rem;\n' +
        '}\n' +
        '.canvas-controls { display: none !important; }\n' +
        '.progress-bar, .slide-counter, .nav-hint, .slide-logo, .slide-footer, .slide-number, .slide-ref { display: none !important; }\n' +
        allStyles + '\n' +
        '</style>\n' +
        '</head>\n<body>\n' +
        allSlides +
  ...
  downloadZIP: async function(options) {
    ...
    try {
      ...
      var commonPathZip = '../common/';
      var themeLinkZip = document.querySelector('link[href*="theme.css"]');
      if (themeLinkZip) {
        commonPathZip = themeLinkZip.getAttribute('href').replace('theme.css', '');
      }
      for (var j = 0; j < this.COMMON_FILES.length; j++) {
        ...
        try {
          var cResp = await fetch(commonPathZip + cFile);
          if (cResp.ok) {
